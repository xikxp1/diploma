...
Таким образом, можно сделать вывод, что для любого $m$, начиная с $6$, хотя бы один из определителей матрицы Гурвица для (\ref{eq:ODE}) отрицателен. Так как, $T_0 = a_0 = \dfrac{\tau^{m-1}}{(m-1)!} >0$ для $\tau>0$. В аналитической части работы данный факт будет аналитически доказан, а также будет доказана теорема о неустойчивости всех приближений (\ref{eq:ODE}) кроме первого.